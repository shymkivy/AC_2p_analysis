function data = f_mpl_preprocess_data2(data, ops)
...
for n_cond = 1:numel(ops.regions_to_analyze)
    ...
    cdata.num_cells = zeros(num_dsets, 1);
    cdata.num_cells_pl = zeros(num_dsets, num_planes);
    cdata.traces_raw = cell(num_dsets, 1);
    cdata.firing_rate = cell(num_dsets, 1);
    cdata.firing_rate_smooth = cell(num_dsets, 1);
    ...
    for n_dset = 1:num_dsets
        ...
        cdata.offset_window_frames{n_dset,1} = and(cdata.trial_window_t{n_dset}>ops.offset_window(1),cdata.trial_window_t{n_dset}<ops.offset_window(2)); 
        

        traces_raw_pl = cell(cdata.num_planes(n_dset),1);
        firing_rate_pl = cell(cdata.num_planes(n_dset),1);
        firing_rate_smooth_pl = cell(cdata.num_planes(n_dset),1);
        cell_plane_indx = cell(cdata.num_planes(n_dset),1);
         % pull out data
        for n_pl = 1:cdata.num_planes(n_dset)
            ...
            traces_raw_pl{n_pl} = if_fill_cuts(traces_raw_cut, cuts_trace);
            firing_rate_pl{n_pl} = if_fill_cuts(firing_rate_cut, cuts_trace);
            firing_rate_smooth_pl{n_pl} = if_fill_cuts(firing_rate_cut_smooth, cuts_trace);
            
            cdata.num_cells_pl(n_dset,n_pl) = size(firing_rate_pl{n_pl},1);
            cell_plane_indx{n_pl} = ones(cdata.num_cells_pl(n_dset,n_pl),1)*n_pl;
            cdata.stim_frame_index{n_dset,n_pl} = cdata.proc_data{n_dset}.stim_frame_index{n_pl};

        end
        cdata.traces_raw{n_dset} = cat(1,traces_raw_pl{:});
        cdata.firing_rate{n_dset} = cat(1,firing_rate_pl{:});
        cdata.firing_rate_smooth{n_dset} = cat(1,firing_rate_smooth_pl{:});
        cdata.cell_plane_indx{n_dset} = cat(1,cell_plane_indx{:});
