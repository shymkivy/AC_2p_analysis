function data_dim_est = f_ensemble_analysis_YS_raster(firing_rate, params)
% parameters
...
var_thresh_prc = f_get_param(params, 'var_thresh_prc', 0); % circular shift thresh (95 or 99; from Detecting cell assemblies in large neuronal populations)
...
%% estimate smooth
if estimate_smooth
    f_ens_smooth_v_dim(raster, num_shuff_reps)
end
...
if sort_cells
    ...
    hclust_out_cell = f_hcluster_wrap(firing_rate, [], hc_params, ops);
    hclust_out_tr = f_hcluster_wrap(firing_rate', [], hc_params, ops);
end

if plot_stuff
    f_plot_raster_mean(firing_rate, 1, [], ops)
    title('raster pre')

    f_plot_raster_mean(firing_rate(hclust_out_cell.dend_order,:), 1, [], ops)
    title('raster cell sort')

    f_plot_raster_mean(firing_rate(hclust_out_cell.dend_order,hclust_out_tr.dend_order), 1, [], ops)
...
%% Visualize traces
scores_3d = reshape(scores, num_ens_comps, num_bins, num_trials);

tn_to_dred = params.tn_to_dred;
tt_to_dred = params.tt_to_dred;
% analyze onset offset angles
on_scores = squeeze(scores_3d(:,params.on_bin,:));
off_scores = squeeze(scores_3d(:,params.off_bin,:));
...
for n_plt = 1:num_plots
    ...
    for ii = 1:3
        plot3(squeeze(scores_3d(dims1(1),:,trial_types_dred == tt_to_dred(ii))), squeeze(scores_3d(dims1(2),:,trial_types_dred == tt_to_dred(ii))),squeeze(scores_3d(dims1(3),:,trial_types_dred == tt_to_dred(ii))), ops.context_colors{ii}); axis tight;
        plot3(on_scores(dims1(1),trial_types_dred == tt_to_dred(ii)), on_scores(dims1(2),trial_types_dred == tt_to_dred(ii)),on_scores(dims1(3),trial_types_dred == tt_to_dred(ii)), 'Color', ops.context_colors{ii}, 'Marker', 'o', 'LineStyle', 'none');
        plot3(off_scores(dims1(1),trial_types_dred == tt_to_dred(ii)), off_scores(dims1(2),trial_types_dred == tt_to_dred(ii)),off_scores(dims1(3),trial_types_dred == tt_to_dred(ii)), 'Color', ops.context_colors{ii}, 'Marker', '*', 'LineStyle', 'none');
        axis tight;
    end
    xlabel(sprintf('pc %d', dims1(1)));
    ylabel(sprintf('pc %d', dims1(2)));
    zlabel(sprintf('pc %d', dims1(3)));
    title(sprintf('%s, dset%d, tca pcs %d %d %d, same freq c r d, o=on, *=off',params.cond_name, params.n_dset, dims1(1), dims1(2), dims1(3)));
...
figure; hold on;
for n_type = 1:3
    tt_list = find(trial_types_dred == tt_to_dred(n_type));
    dist1 = zeros((numel(tt_list)-1),1);
    for n_rep = 1:(numel(tt_list)-1)
        dist1(n_rep) = (on_scores(:,tt_list(n_rep))/norm(on_scores(:,tt_list(n_rep))))'*(on_scores(:,tt_list(n_rep+1))/norm(on_scores(:,tt_list(n_rep+1))));
    end
    [f, x] = ecdf(dist1);
    plot(x, f, ops.context_colors{n_type}, 'lineWidth', 2);
end
for n_type = 1:3
    ...
    for n_rep = 1:num_rep
        ...
        dist1(n_rep,n_type) = (on_scores(:,samp_tr(1))/norm(on_scores(:,samp_tr(1))))'*(on_scores(:,samp_tr(2))/norm(on_scores(:,samp_tr(2))));
    end
    [f, x] = ecdf(dist1(:,n_type));
    plot(x, f, ops.context_colors{n_type}, 'LineStyle', '--', 'lineWidth', 2);
end
...
for n_type = 1:3
    ...
    for n_rep = 1:num_rep
        samp_tr = randsample(tt_list,2);
        dist1(n_rep,n_type) = (off_scores(:,samp_tr(1))/norm(off_scores(:,samp_tr(1))))'*(off_scores(:,samp_tr(2))/norm(off_scores(:,samp_tr(2))));
    end
    [f, x] = ecdf(dist1(:,n_type));
    plot(x, f, ops.context_colors{n_type}, 'LineStyle', '--', 'lineWidth', 2);
end
for n_type = 1:3
    ...
    dist1 = zeros((numel(tt_list)-1),1);
    for n_rep = 1:(numel(tt_list)-1)
        dist1(n_rep) = (off_scores(:,tt_list(n_rep))/norm(off_scores(:,tt_list(n_rep))))'*(off_scores(:,tt_list(n_rep+1))/norm(off_scores(:,tt_list(n_rep+1))));
    end
    [f, x] = ecdf(dist1);
    plot(x, f, ops.context_colors{n_type}, 'lineWidth', 2);
end
title(sprintf('%s, dset%d, Offset population trajectory structure ECDF',params.cond_name, params.n_dset));
legend('cont', 'red', 'dev', 'rand', 'Location', 'northwest');
xlabel('cosine similarity betweeen trials');
ylabel('Fraction');
...
if strcmpi(ensamble_method, 'tca')
    ...
    trial_types_dred_reg_colors = zeros(numel(trial_types_dred),3);
    for n_tt = 1:numel(tt_to_dred)
        trial_types_dred_reg_colors(trial_types_dred == tt_to_dred(n_tt),:) = trial_types_dred_reg_colors(trial_types_dred == tt_to_dred(n_tt),:)+c_map(n_tt,:);
    end
    trial_image_reg = reshape(trial_types_dred_reg_colors,1,numel(trial_types_dred),3);
...
if plot_ens_details
    ...
    if numel(intersect(params.ctx_mmn,trial_types_dred))>= 6
        ...
        % extract ctx trials
        trial_types_ctx_index = logical(sum(trial_types_dred == params.ctx_mmn',2));
        trial_types_ctx = trial_types_dred(trial_types_ctx_index);
    ...
    cont_trials = intersect(1:10,trial_types_dred);
    if numel(cont_trials)
        c_map_cont = jet(numel(cont_trials));
        trial_types_cont_index = logical(sum(trial_types_dred == 1:10,2));
        trial_types_cont = trial_types_dred(trial_types_cont_index);
    ...
    for n_ens = 1:numel(ens)
        ...
        if numel(cont_trials)>2
            ...
            ens_data = reshape(firing_rate(ens(n_ens).ens_cell_num,:),numel(ens(n_ens).ens_cell_num), [], num_trials);
            trial_ave = f_mpl_trial_average(ens_data,trial_types_dred, cont_trials, 'none');
        ...
        if numel(intersect(params.ctx_mmn,trial_types_dred))>= 6
            ...
            ens_data = reshape(firing_rate(ens(n_ens).ens_cell_num,:),numel(ens(n_ens).ens_cell_num), [], num_trials);
            trial_ave = f_mpl_trial_average(ens_data,trial_types_dred, params.ctx_mmn, 'none');
