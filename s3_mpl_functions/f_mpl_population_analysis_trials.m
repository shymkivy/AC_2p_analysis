...
for n_cond = 1:numel(ops.regions_to_analyze)
    ...
    for n_dset = 1:cdata.num_dsets
        ...
        num_cell_plane = zeros(sum(cdata.num_cells(n_dset,:)),1);
        curr_cell = 1;
        ...
        for n_pl = 1:cdata.num_planes(n_dset)
            ...
            num_cell_plane(curr_cell:curr_cell+cdata.num_cells(n_dset,n_pl)-1) = n_pl;
            curr_cell = curr_cell + cdata.num_cells(n_dset,n_pl);
...
%trial_data_2d = reshape(trial_data,num_cells,[]);
        ...
        if 1
            ...
            title(sprintf('trial-trial similarity %s dset %d', cond_name, n_dset));
            xlabel('dev trial');
            ylabel('dev trial');
            Y = tsne(x');
            figure;
            gscatter(Y(:,1),Y(:,2),clust_ident);
            title(sprintf('%s dset %d, T-SNE', cond_name, n_dset));
