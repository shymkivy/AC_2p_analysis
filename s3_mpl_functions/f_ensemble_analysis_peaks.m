function data_dim_est = f_ensemble_analysis_peaks(trial_data,trial_types_dred, params, ops)
% parameters
...
for n_plt = 1:num_plots
    ...
    for n_tt = 1:numel(tt_to_dred)
        plot3(X(trial_types_dred == tt_to_dred(n_tt),dims1(1)), X(trial_types_dred == tt_to_dred(n_tt),dims1(2)),X(trial_types_dred == tt_to_dred(n_tt),dims1(3)), 'Color', ops.colors_list{n_tt},'Marker', 'o', 'LineStyle', 'none'); 
        axis tight;
...
figure; hold on;
for n_tt = 1:numel(tt_to_dred)
    tt_list = find(trial_types_dred == tt_to_dred(n_tt));
