function info = viz_ktensor_mmn(K, trial_image, varargin)
...
for k = 1 : nd
    ...
    for j = 1 : nc
        ...
        switch res.Plottype{k}
            case 'line'
                if k == 3
                    imagesc(FactorAxes(k,j),1:size(trial_image,2), -0.5:0.1:0.5, trial_image);
                    hold(FactorAxes(k,j), 'on');
                    plot(FactorAxes(k,j), xx, yy, 'Linewidth', lw, 'Color', cc);
                end
