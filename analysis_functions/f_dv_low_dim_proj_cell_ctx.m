...
if ~strcmpi(params.trial_type, 'Context') % do context
...
for n_dset = 1:num_dsets
    ...
    if ~(sum(cont_tn_all > 10) || sum(cont_tn_all < 1))
        [sel_cells, resp_vals, resp_vals2, ~, resp_cells] = f_dv_get_resp_vals_cells(stats1, tn_all2, params);
...
[lr_data, residual_var, residual_var_pca] = f_dv_run_dred2(data_all2', params.dim_red_method, params.distance_method);
...
if num_dims >= 2
    ...
    for n_cell = 1:numel(cell_labels2)
        if cell_labels2(n_cell)
            color1 = ops.context_types_all_colors2{tn_all2(cell_labels2(n_cell))};
        else
            color1 = [.6 .6 .6];
        end
    ...
    f_dv_plot2_pc(data_mean, tn_all2, [], title_tag2, ops.context_types_all_colors2, {cont_idx, ctx_idx})
    %f_dv_plot3_pc(data_mean, tn_all, [], title_tag3, ops.context_types_all_colors2, {cont_idx, ctx_idx})
...
if num_dims >= 3
    ...
    %f_dv_plot2_pc(data_mean, tn_all2, [], title_tag2, ops.context_types_all_colors2, {cont_idx, ctx_idx})
    f_dv_plot3_pc(data_mean, tn_all2, [], title_tag2, ops.context_types_all_colors2, {cont_idx, ctx_idx})
end
end