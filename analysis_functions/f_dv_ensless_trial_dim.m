...
% this will compute dim
params = f_dv_gather_params(app);
ddata = app.ddata;
ops = app.ops;
% ensionality of trials
...
if ~isempty(ddata.ensembles{1})
    ens_list = ddata.ensembles{1}.ens_out.cells.ens_list(ddata.ensemble_stats{1}.accepted_ensembles);
...
for n_tt = 1:num_tn
    ...
    resp_cells = f_dv_get_resp_vals_cells(stats1, tn, params);
    
    if ~isempty(ddata.ensembles{1})
        resp_ens = logical(sum(ddata.ensemble_tuning_stats{1}.peak_resp_cells(:,tn),2));
