...
frac = params.plot_fractions;

num_tn = numel(tn_all);
num_dsets = numel(data.experiment);
reg_all = ops.regions_to_analyze;

categories = ops.context_types_labels(tn_all);
...
for n_dset = 1:num_dsets
    ...
    [~, trial_types_wctx] =  f_s3_add_ctx_trials([], trial_types, ddata.MMN_freq{1}, ops);

    stim_all = stim_all + sum(trial_types_wctx == ops.context_types_all(tn_all)',1)';
end
figure;
...
if params.pool_regions
    ...
    for n_br = 1:numel(br1)
        br1(n_br).FaceColor = ops.cond_colors{n_br};
    end
...
if params.pool_regions
    ...
    for n_br = 1:numel(reg_all)
        ...
        br1.FaceColor = ops.cond_colors{n_br};
    end
    ylabel('Cell fraction');
    %legend(reg_all, 'location', 'northwest');
end
