...
plot_split_mmn = 1;
num_red = 10;
alpha1 = params.stim_transparancy;
color1 = ops.context_types_all_colors2{params.stim_freq_color};
...
if ~sum(strcmpi(params.trial_type, {'Context', 'Context_flip', 'Context_both_comb'}))
    params.trial_type = 'Context_both_comb';
end

[num_gr, num_tn] = size(tn_all);

vol_per = data.proc_data{1}.frame_data.volume_period_ave;
...
if num_reg>1
    col_reg = ops.cond_colors;
else
    col_reg = ops.context_colors(2);
end
...
for n_dset = 1:num_dsets
    ...
    for n_gr = 1:num_gr
        ...
        tn1 = tn_all(n_gr, :);
        num_tn = numel(tn1);
        red_start = ops.context_types_all(tn1(1) - 7);
        red_end = ops.context_types_all(50 - tn1(3));
        ...
        for n_reg = 1:num_reg
            ...
            if num_cells
                ...
                for n_tn = 1:num_tn
                    ctx1 = ops.context_types_all(tn1(n_tn));
...
title_tag2 = sprintf('%s; %s; %s; %s reg', title_tag, params.responsive_cells_select, params.trial_type, reg_tag);
...
ymin = min([ymin, params.min_y_lim]);
ymax = max([ymax, params.max_y_lim]);
    ...
    if num_cells
        ...
        if combine_reg
            if num_reg>1
                ...
                color2 = ops.context_colors{2};
            end
        else
            color2 = ops.context_types_all_colors2{19};
        end
        ...
        if plot_split_mmn
            ...
            for n_tn = 1:num_tn
                ...
                color2 = ops.context_types_all_colors2{tt1};
...
if strcmpi(reg_tag, 'All comb')
    ...
    colors1 = ops.cond_colors;
    colors2 = ops.cond_colors;
    colors3 = ops.cond_colors;
end
