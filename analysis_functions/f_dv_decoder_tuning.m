...
onset_frames = and(plot_t >= params.onset_window(1), plot_t <= params.onset_window(2));
offset_frames = and(plot_t >= params.offset_window(1), plot_t <= params.offset_window(2));
...
if numel(dec_params.num_cells) > 1
    f_plot_cond_decoding(dec_data_out, 'num_cells', dec_params, ops, title_tag);
    if numel(dec_params.decoder_type) > 1
        f_plot_decoret_types(dec_data_out, 'num_cells', dec_params, ops, title_tag);
    end
end
