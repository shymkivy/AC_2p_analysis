...
resp_cells = f_dv_get_resp_vals_cells(stats1, tn_all, params);
...
if isempty(tn_all)
    ...
    if isempty(tt_all)
        ...
        [~, trial_frames] = f_dv_compute_window_t(params.trial_window, cdata(1).volume_period);
        ...
        tn_last_seq = f_tt_to_tn(tr_last2, ops, 1);

        if params.sort_by_trial_type
            if params.sort_by_prev_trial
        ...
        if ~isempty(mmn_freq)
            title_tag2 = sprintf('%s raster; mmn %d %d', ddata.dset_name_full{1}, mmn_freq(1), mmn_freq(2));
        else
            title_tag2 = sprintf('%s raster', ddata.dset_name_full{1});
        end
        ...
        f_plot_raster_mean(firing_rate3(ord_cell,:), 1, raster_t, tn_seq_plot, ops.context_types_all_colors2, params.colormap, params.invert_cmap);
        sgtitle(title_tag2, 'interpreter', 'none');
        % clim1 = clim;
        % clim([0 clim1(2)]);

        if params.plot_pop_vector
            ...
            for n_tn = 1:numel(tt_cont)
                ...
                idx3 = ops.context_types_all == tt_cont(n_tn);
                plot(ones(num_cells,1)*n_tn + trial_ave1n(ord_cell, idx2), 1:num_cells, color=ops.context_types_all_colors2{idx3}, LineWidth=2)
            ...
            tt_mmn = tt_all(logical(sum(tt_all' == ops.context_types_all([18, 19,20, 28, 29, 30])',2)));
            f1 = figure; hold on;
            for n_tn = 1:numel(tt_mmn)
                idx2 = tt_all == tt_mmn(n_tn);
                idx3 = find(ops.context_types_all == tt_mmn(n_tn),1);
                plot(ones(num_cells,1)*n_tn + trial_ave1n(ord_cell, idx2), 1:num_cells, color=ops.context_types_all_colors2{idx3}, LineWidth=2)
                plot(ones(num_cells,1)*n_tn, 1:num_cells, color=[0.3 0.3 0.3 0.5], LineWidth=1, LineStyle=':')
            end
            f1.Children.YDir = 'reverse';
