...
[region_num, reg_tag, leg_list, reg_col] = f_dv_get_region_sel_val(params, ops);
...
%%
title_tag2 = sprintf('%s; %s; %s; %s', params.responsive_cells_type, params.responsive_cells_select, title_tag, reg_tag);

if params.render_painters
...
for n_reg = 1:num_reg
    ...
    plot3(features_proc31, features_proc32, features_proc33, '.', MarkerSize=markSize, color=reg_col{n_reg})
end
...
[gr_id, gr_tag] = f_dv_combine_data(mouse_id, dset_id, params);
gr_all = unique(gr_id);
    ...
    for n_pl = 1:num_pl
        ...
        for n_dgr = 1:num_dgr
            ...
            if num_cells > 5
                ...
                dist2(n_dgr) = dist1/sqrt(num_cells)*sqrt(50);
            end
        end
...
for n_pl = 1:num_pl
    ...
    for n_reg = 1:num_reg
        ...
        plot(features_proc31, features_proc32, '.', MarkerSize=markSize, color=reg_col{n_reg});
...
f_plot_bar(cos_si_all, bar_leg, ops.context_types_all_colors2(tn_all(1,:)), ops.cond_colors);
ylabel('Cosine similarity')
...
f_plot_bar(dist_all, bar_leg, ops.context_types_all_colors2(tn_all(1,:)), ops.cond_colors);
ylabel('Normalized euclidean distance')
title(title_tag2, 'interpreter', 'none')
