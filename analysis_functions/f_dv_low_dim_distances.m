...
title_tag1 = sprintf('%s; dist %s', title_tag, params.distance_method);
...
for n_dset = 1:num_dsets
    ...
    [~, resp_vals] = f_dv_get_resp_vals_cells(stats1, tn_all, params);
...
for n_dset = 1:num_dsets
    ...
    for n_mmn = 1:num_mmn
        for n_tnd = 1:num_tnd
            ...
            dist_all{n_dset, n_mmn, n_tnd} = pdist2(cont_vec', vec1', params.distance_method);
        end
    end
end
...
for n_tnd = 1:num_tnd
    ...
    plot(x_lab, dist_temp, '-', 'color', [ops.context_types_all_colors2{tn_d(1, n_tnd)} 0.2])
    plot(x_lab, mean(dist_temp,2), 'o-', 'color', ops.context_types_all_colors2{tn_d(1, n_tnd)}, 'linewidth', 2)

    plot(x_lab, dist_temp_opp, '--', 'color', [ops.context_types_all_colors2{tn_d(1, n_tnd)}, 0.2])
    plot(x_lab, mean(dist_temp_opp,2), 'o--', 'color', ops.context_types_all_colors2{tn_d(1, n_tnd)}, 'linewidth', 2)
end
xlabel('Frequency around MMN');
ylabel([params.distance_method ' distance']);
title(title_tag1)
