...
tn_all = f_dv_get_trial_number(params, ddata.MMN_freq{1});
...
num_pl_d = str2double(params.num_axes_plot);
num_pl = floor(params.plot_pca_dim/num_pl_d);
...
[plot_t, trial_frames] = f_dv_compute_window_t(params.trial_window, app.ddata.proc_data{1}.frame_data.volume_period_ave);
    ...
    for n_tngr = 1:num_tn_gr
        ...
        for n_reg = 1:num_reg
            ...
            if num_cells
                ...
                for n_tn = 1:num_tn
                    idx1 = logical(sum([trial_types, trial_types_ctx] == ops.context_types_all(tn1(n_tn)),2));
...
for n_dset = 1:num_dsets
    ...
    for n_reg = 1:num_reg
        ...
        if num_cells
            ...
            top_comp = score(:,1:params.plot_pca_dim);
            top_comp2 = reshape(top_comp, [num_t, sum(resp_num_trials3), params.plot_pca_dim]);
