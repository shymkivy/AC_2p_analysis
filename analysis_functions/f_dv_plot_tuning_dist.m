...
categories = ops.context_types_labels(tn_all(1,:));

stim_all = zeros(num_gr, num_dsets, num_tn);
for n_dset = 1:num_dsets
    data1 = data(n_dset,:);
    trial_types = data1.trial_types{1};
    trial_types_ctx = f_dv_mark_tt_ctx(trial_types, data1.MMN_freq{1}, ops);
    
    trial_types2 = [trial_types, trial_types_ctx];
    for n_gr = 1:num_gr
        stim_all(n_gr, n_dset, :) = squeeze(sum(sum(trial_types2 == reshape(ops.context_types_all(tn_all(n_gr,:)), 1, 1, []),1),2));
    end
end
...
for n_dset = 1:num_dsets
    ...
    if ~params.plot_ens
        stats1 = cat(1,data1.stats{params.planes});
    else
        stats1 = cat(1,data1.ensemble_tuning_stats{params.planes});
    end
...
f1.Children.YLim(2) = max([max_y, params.max_y_lim]);
legend(b1, leg_list2)
...
for n_reg = 1:num_reg
    ...
    for n_tn = 1:num_tn
        plot(num_cells4, tun_data2(:, n_reg, n_tn), '.', 'color', ops.context_types_all_colors2{tn_all(1,n_tn)})
    end
end
