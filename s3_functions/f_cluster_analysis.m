function f_cluster_analysis(data, ops)
    for n_flip1 = 1:numel(ops.flip_to_analyze)
        for n_cond = 1:numel(ops.conditions_to_analyze)
            ...
            if n_flip == 3
                ...
                ctx_traces = ctx_traces(ctx_cells,ops.win.analysis_window,:);
            else

                % contex responsive cells (num_cells, frames, context, flip)
                ctx_cells = data.(cond_name).context_resp_cells(:,n_flip);
                ctx_traces = data.(cond_name).trial_ave_z_mmn(ctx_cells,ops.win.analysis_window,:,n_flip);
            end
    ...
    %         title(sprintf('%s, %s: Clustering results', ops.conditions{n_cond}, ops.fig_title_run{n_flip}));
            ...
            stat_all_onset_averages = squeeze(mean(ctx_traces(:,ops.win.onset_window_frames,:),2));
            ...
            for n_cntxt = 1:3
                ...
                shadedErrorBar(ops.time_stim_window(ops.win.analysis_window), (traces_ave(:,n_cntxt)), traces_SEM(:,n_cntxt), 'lineprops', ops.context_colors{n_cntxt});
            end
            ...
            for n_clust = 1:ops.num_clusters(n_cond)
                ...
                % more statistics
                stat_clust_onset_averages = squeeze(mean(ctx_traces(icx_final==n_clust,ops.win.onset_window_frames,:),2));
            ...
            for ii = 1:3
                subplot(1,3,ii);
                imagesc(ops.analysis_t,1:size(ctx_traces_sort,1),ctx_traces_sort(:,:,ii));
                colormap gray;
                caxis([0 6]);
