function data_smooth = f_smooth_gauss(data, smoothSDbinRatio)

[num_cells, num_bins, num_trials] = size(data);

%% Smooth data
% compute the size of smoothing window
kernel_half_size = ceil(sqrt(-log(0.05)*2*smoothSDbinRatio^2));
gaus_win = -kernel_half_size:kernel_half_size;
gaus_kernel = exp(-((gaus_win).^2)/(2*smoothSDbinRatio^2));
