...
corr_vals = cell(num_dsets, num_tn);
...
for n_dset = 1:num_dsets
    ...
    for n_tn = 1:num_tn
        ...
        if min_num_cells >=2
            ...
            corr_vals{n_dset, n_tn} = mean(SI_all,2);
        end
    end
end
fprintf('\n');
...
for n_tn = 1:num_tn
    corr_vals2 = nan(num_dsets, num_x);
    for n_dset = 1:num_dsets
        num_val = numel(corr_vals{n_dset, n_tn});
        corr_vals2(n_dset, 1:num_val) = corr_vals{n_dset, n_tn};
    end
    for n_x = 1:num_x
        idx1 = ~isnan(corr_vals2(:, n_x));
        if sum(idx1)
            temp_vals = corr_vals2(idx1, n_x);
