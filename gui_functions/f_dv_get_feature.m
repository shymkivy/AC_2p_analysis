function [features, sel_cells, area_labels, features_proc] = f_dv_get_feature(app, feature, tn_all, resp_cell_select)

[data, ~] = f_dv_get_data_by_mouse_selection(app);

[region_num, ~, ~] = f_dv_get_region_sel_val(app);
...
for n_dset = 1:num_dsets
    ...
    for n_gr = 1:num_gr
        ...
        tn_all2 = tn_all(n_gr,:);
        [sel_cells_peak, ~, vals_peak] = f_dv_get_resp_vals_cells(app, stats1, tn_all2, 'peaks', resp_cell_select);
        [sel_cells_onset, ~, vals_onset] = f_dv_get_resp_vals_cells(app, stats1, tn_all2, 'onset', resp_cell_select);
        [sel_cells_offset, ~, vals_offset] = f_dv_get_resp_vals_cells(app, stats1, tn_all2, 'offset', resp_cell_select);
        
        % get area labels
        area_labels0 = f_dv_get_area_label(app, data(n_dset,:));
