...
title_tag2 = sprintf('%s, %s', title_tag, app.ResponsivecellsselectDropDown.Value);
...
for n_dset = 1:num_dsets
    ...
    if ~isempty(ddata.registered_data{n_pl})
        ...
        for n_gr = 1:num_gr
            tn1 = tn_all(n_gr,:);
            [resp_cells, ~, resp_vals_full] = f_dv_get_resp_vals_cells(app, stats1, tn1);
...
for n_tn = 1:num_tn
    color1 = 1 - app.ops.context_types_all_colors2{tn1(n_tn)};
...
for n_tn = 1:num_tn
    ...
    color1 = app.ops.context_types_all_colors2{tn1(n_tn)};
    for n_cell = 1:num_cells
        if resp_cells_all2(n_cell, n_tn)
