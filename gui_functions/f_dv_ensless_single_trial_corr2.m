...
for n_dset = 1:num_dsets
    ...
    for n_tn = 1:num_tn
        ...
        if num_cells2>1
            ...
            tr_ave = mean(tr_data_2d_tr,2);

            if strcmpi(app.corr2metricsDropDown.Value, 'cosine')
                ytag = 'cosine similarity';
                SI_vals = 1 - pdist(tr_data_2d_tr', 'cosine');
            elseif strcmpi(app.corr2metricsDropDown.Value, 'cosine_from_mean')
                SI_vals = 1 - pdist2(tr_ave', tr_data_2d_tr', 'cosine');
                ytag = 'cosine similarity';
            elseif strcmpi(app.corr2metricsDropDown.Value, 'std_from_0')
                SI_vals = sqrt(mean((tr_data_2d_tr).^2,1));
                ytag = 'normalized frobenius norm';
            elseif strcmpi(app.corr2metricsDropDown.Value, 'std_from_mean')
                SI_vals = sqrt(mean((tr_data_2d_tr - tr_ave).^2,1));
                ytag = 'variance';
            end
...
title_tag2 = sprintf('%s; %s', title_tag, app.corr2metricsDropDown.Value);
...
xlabel('num cells'); ylabel(ytag)
title(sprintf('Mean pairwise correlations; %s', title_tag2), 'interpreter', 'none');
...
ylabel(ytag);
title(sprintf('Mean correlation across datasets, %s', title_tag2), 'interpreter', 'none')
...
title(sprintf('Numebr of cells across datasets, %s', title_tag2), 'interpreter', 'none')
